...
export class PoolApp {
  ...
  focusCameraOnWall(wall) {
    ...
    const standoff = Math.max(1.2, Math.min(Math.max(poolBounds.getSize(new THREE.Vector3()).x, poolBounds.getSize(new THREE.Vector3()).y) * 0.18, 2.8));
    const eyeHeight = Math.max(0.7, Math.min(1.35, wallSize.z * 0.55));
    const targetHeight = Math.max(-0.15, Math.min(0.7, wallBounds.max.z - wallSize.z * 0.22));
